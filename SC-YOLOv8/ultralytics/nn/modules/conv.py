...
__all__ = (
    ...
    "CBAM",
    "Concat",
    "RepConv","DySnakeConv","DSConv","DSC","LAWDS","CoordAtt"
)
...
#         self.conv_hw = GhostConv(in_channels, out_channels, (3, 1))
#         self.conv_pool_hw = GhostConv(in_channels, out_channels, 1)
#
#     def forward(self, x):
#
#         _, _, h, w = x.size()
#         x_h, x_w = self.pool_h(x), self.pool_w(x).permute(0, 1, 3, 2)
#         x_cat_hw = torch.cat([x_h, x_w], dim=2)
#         x_cat_hw = self.conv_hw(x_cat_hw)
#
#         x_h, x_w = torch.split(x_cat_hw, [h, w], dim=2)
#         x_pool_hw_weight = self.conv_pool_hw(x_cat_hw).sigmoid()
#         x_pool_h_weight, x_pool_w_weight = torch.split(x_pool_hw_weight, [h, w], dim=2)
#         x_h, x_w = x_h * x_pool_h_weight, x_w * x_pool_w_weight
#
#         return x * x_w * x_h
...
class LAWDS(nn.Module):
    ...
    def __init__(self, c1, c2, group=16) -> None:
        ...
        self.ds_conv = Conv(c1, c2 * 4, k=3, s=2, g=(c1 // group))
